...
SCENE_TYPE_DESCRIPTIONS = {
    ...
    "转折场景": "剧情在此处发生180度逆转，读者预期被彻底打破，主角或反派处境骤然反转",
    ...
    def _semantic_verify_case(
        ...
        for stype, anchor_vec in anchors.items():
            ...
            if sim > best_sim:
                best_sim = sim
                best_type = stype
            if stype == keyword_scene_type:
                orig_sim = sim

        if best_sim < min_similarity:
            return None  # 语义相关度太低，丢弃

        # 只有当语义差值明显（>0.08）才允许改类型，避免引力黑洞效应
        if best_type != keyword_scene_type and (best_sim - orig_sim) < 0.08:
            best_type = keyword_scene_type
